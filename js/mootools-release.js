...
}}}};window.onDomReady=function(A){return this.addEvent("domready",A);};var Drag={};Drag.Base=new Class({options:{handle:false,unit:"px",onStart:Class.empty,onBeforeStart:Class.empty,onComplete:Class.empty,onSnap:Class.empty,onDrag:Class.empty,limit:false,modifiers:{x:"left",y:"top"},grid:false,snap:6},initialize:function(B,A){this.setOptions(A);
...
A=B;break;default:C=B.hexToRgb(true);A=C.rgbToHsb();}C.hsb=A;C.hex=C.rgbToHex();return $extend(C,Color.prototype);},mix:function(){var A=$A(arguments);
...
case 4:return[B,F,C];case 5:return[C,F,D];}}return false;}});
