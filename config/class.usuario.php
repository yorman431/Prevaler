		...
		if (isset($_POST['envio']) && $_POST['envio']=="Guardar"){
			...
			if($this->clave!=$this->confirmar){
				...
				if($resultado=mysql_fetch_array($consulta)){
					...
					$sql="INSERT INTO usuario VALUES ('', '$this->nombre', '$this->apellido', '$this->correo', '$this->login', '$this->clave', '$this->nivel')";
					$consulta=mysql_query($sql) or die(mysql_error());
		...
		if (isset($_POST['envio']) && $_POST['envio']=="Guardar" && isset($_GET['id']) && $_GET['id']!=""){
			...
			if($this->clave!=$this->confirmar){
				...
				if($resultado=mysql_fetch_array($consulta)){
					...
					$sql="UPDATE usuario SET nombre_uso='$this->nombre', apellido_uso='$this->apellido', correo_uso='$this->correo', nivel_uso='$this->nivel', login_uso='$this->login', clave_uso='$this->clave' WHERE id_uso='$id'";
					$consulta=mysql_query($sql) or die(mysql_error());
					header("location:/admin/usuario/");
				}
			}
