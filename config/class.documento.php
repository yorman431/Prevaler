...
class Documento{
	...
	function listar_documento(){
		...
		if (isset($_SESSION['buscar']) && $_SESSION['buscar']!=""){
			...
			$sql="SELECT * FROM documento, categoria WHERE id_cat=categoria_doc AND
			...
			$sql="SELECT * FROM documento, categoria WHERE id_cat=categoria_doc ORDER BY id_doc DESC";
		}
		...
		while ($resultado = mysql_fetch_array($consulta)){
			...
			$this->buscar_ruta_nodo($resultado['categoria_doc']);
	...
	function listar_documento_publico($id){
		/* Metodo para listar los usuarios y sus opciones. */
		if (isset($_POST['buscar']))
			$_SESSION['buscar']=$_POST['buscar'];
			
		if($id!=0) $restriccion="id_cat=categoria_doc AND categoria_doc='$id'";
		else $restriccion="id_cat=categoria_doc";
		...
		if (isset($_SESSION['buscar']) && $_SESSION['buscar']!=""){
			...
			$sql="SELECT * FROM documento, categoria WHERE $restriccion ORDER BY id_doc DESC";
		}
		$consulta=mysql_query($sql);
		while ($resultado = mysql_fetch_array($consulta)){
			$this->mensaje="si";
	...
	function mostrar_documento(){
		...
		if (isset($_GET['id']) && $_GET['id']!=""){
			...
			$this->id_cat=$resultado['categoria_doc'];
				...
				if (is_uploaded_file($_FILES['documento']['tmp_name'])) {	
					...
					if ($_FILES['documento']['type'] == "application/msword" || $_FILES['documento']['type'] == "application/pdf" || $_FILES['documento']['type'] == "application/octet-stream" || $_FILES['documento']['type'] == "application/vnd.ms-excel" || $_FILES['documento']['type'] == "image/jpeg" || $_FILES['documento']['type'] == "image/pjpeg" || $_FILES['documento']['type'] == "image/png" || $_FILES['documento']['type'] == "image/x-png"){
						...
						if ($_FILES['documento']['size']>=2048000){
							...
							$consulta=mysql_query("INSERT INTO documento (id_doc, categoria_doc, titulo_doc, fecha_doc, tipo_doc, peso_doc, nombre_doc, contenido_doc) VALUES ('', '$this->categoria', '$this->titulo','$this->fecha','$tipo','$peso','$archivo','$this->contenido')") or die(mysql_error());
				...
				if (is_uploaded_file($_FILES['documento']['tmp_name'])) {	
					...
					if ($_FILES['documento']['type'] == "application/msword" || $_FILES['documento']['type'] == "application/pdf" || $_FILES['documento']['type'] == "application/octet-stream" || $_FILES['documento']['type'] == "application/vnd.ms-excel" || $_FILES['documento']['type'] == "image/jpeg" || $_FILES['documento']['type'] == "image/pjpeg" || $_FILES['documento']['type'] == "image/png" || $_FILES['documento']['type'] == "image/x-png"){
						...
						if ($_FILES['documento']['size']>=1024000){
							...
							//guardamos el nuevo
							$this->contenido=html_entity_decode($this->contenido);
							$consulta=mysql_query("UPDATE documento SET categoria_doc='$this->categoria', titulo_doc='$this->titulo', fecha_doc='$this->fecha', tipo_doc='$tipo', peso_doc='$peso', nombre_doc='$archivo', contenido_doc='$this->contenido' WHERE id_doc='$id'") or die(mysql_error());
							//retornamos
							header("location:/admin/documento/");
							exit();
						}
						...
						$this->contenido=html_entity_decode($this->contenido);
						$consulta=mysql_query("UPDATE documento SET categoria_doc='$this->categoria', titulo_doc='$this->titulo', fecha_doc='$this->fecha', contenido_doc='$this->contenido' WHERE id_doc='$id'") or die(mysql_error());
						//retornamos
						header("location:/admin/documento/");
						exit();
				}//fin file upload.
