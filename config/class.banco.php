	...
	/*Metodo para editar un usuario seleccionado */		
		...
		if (isset($_POST['envio']) && $_POST['envio']=="Guardar" && isset($_GET['id']) && $_GET['id']!=""){
			...
			if($resultado=mysql_fetch_array($consulta)){
				...
				$sql="UPDATE banco SET nombre_ban='$this->rif', nombre_ban='$this->nombre', cuenta_ban='$this->cuenta', numero_ban='$this->numero', titular_ban='$this->titular' WHERE id_ban='$id'";
				$consulta=mysql_query($sql) or die(mysql_error());
				header("location:/admin/banco/");
			}
		}else{
