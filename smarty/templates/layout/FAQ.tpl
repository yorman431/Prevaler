            ...
            <ul class="nav" id="side-menu">
              ...
              {section i $faq}
                ...
                {if $faq[i].lista eq $faq[i].id_faq}
                    <a  id="{$faq[i].id_faq}" onclick="return faq();" style="cursor:pointer;" {if $cont eq '1' } class="no-push btn-push" {else} class="no-push"{/if}><i class="fa fa-circle" aria-hidden="true"></i>{$faq[i].pregunta_faq}<i class="fa fa-arrow-right" aria-hidden="true"></i></a>
                {else}
                    <a  id="{$faq[i].id_faq}" onclick="return faq2();" style="cursor:pointer;"><i class="fa fa-circle" aria-hidden="true"></i>{$faq[i].pregunta_faq}</a>
                    <ul class="nav nav-second-level">
                       {section name=j loop=$faq[i].lista}
                        <li>
                        <a  id="{$faq[i].lista[j].id_faq}" onclick="return faq();" style="cursor:pointer;"><i class="fa fa-circle" aria-hidden="true"></i>{$faq[i].lista[j].pregunta_faq}<i class="fa fa-arrow-right" aria-hidden="true"></i></a>
                        </li>
                       {/section}
                    </ul>
                {/if}
